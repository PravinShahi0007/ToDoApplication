...
case "MovingInfoPopupID":b.MovingInfoPopup=$("#"+a.GanttChartOptions.MovingInfoPopupID);case "ConnectingInfoPopupID":b.ConnectingInfoPopup=$("#"+a.GanttChartOptions.ConnectingInfoPopupID);default:b[c]=a.GanttChartOptions[c]}delete a.GanttChartOptions;return b};var e=this.PrepareGanttChartOptions(d);$.extend(!0,a,d);deserializejsfunctionAndCallJSFunction(b.BeforeGanttWidgetInitialitionCallback);a.GanttTableRowLoadedCallBack=deserializejsfunction(b.GanttTableRowLoadedCallBack);a.CustomProgressCalculation=
deserializejsfunction(b.CustomProgressCalculation);a.ProvideWBSID=deserializejsfunction(b.ProvideWBSID);f.OptionsCreated(a,e);deserializejsfunctionAndCallJSFunction(b.AfterGanttWidgetInitializedCallback)});return f.CurrentOptions}else alert("jQuery UI not loaded")}function deserializejsfunctionAndCallJSFunction(b,c){if(b){var a=deserializejsfunction(b);if(a&&(a=a(c)))return a}if(null!=c)return c}
function deserializejsfunction(b){if(b)return jQuery.isFunction(b)?b:deserializejsfunction(window[b])}function deserializeObject(b){if(b)return"object"===typeof b?b:deserializeObject(window[b])}
...
(a.TaskTooltipTemplate=$("#"+a.TaskTooltipTemplateID));a.TimeScaleHeaders&&this.InitTimeScalHeaders(a);a.SpecialLineInfos&&this.InitSPL(a);a.OnTaskBarLoad&&(a.OnTaskBarLoad=deserializejsfunction(a.OnTaskBarLoad))};this.InitOptions(c);return c}
