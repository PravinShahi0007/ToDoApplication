...
function AfterFindEmp(EmpKy) {
    ...
    $.ajax({
        ...
        success: function (data) {
            $("#Ol_AcadQualGrid").data("kendoGrid").dataSource.data(data);
        },
        error: function (e) {
            return false;
        }
...
function LoadDropDown() {
    $("#cmbSubject").kendoComboBox({
        dataValueField: "CdKy",
        dataTextField: "CdNm",
        dataSource: GetCdMas_LookupWeb('O/LSub'),
        filter: "contains",
        suggest: true,
...
function LoadAcadamicDetailGrid() {//alert(0)
    var dataSource = new kendo.data.DataSource({
        batch: true,
        pageSize: 10,
        schema: {
...
function InsertToGrid() {
    var grid = $("#Ol_AcadQualGrid").data("kendoGrid");
